...
##add 800000000 to each tree state number so that the tree can be added to YFV_cauchy_RRW_north2.trees 
...
file = open("/output/YFV_output/north_transfered", "w")
for i in range(len(df)):
    file.write(df[0][j] + ' ' + df[1][j] +'\n')
